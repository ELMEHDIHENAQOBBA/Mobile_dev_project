...
class AuthState with _$AuthState {
  ...
  const factory AuthState.authenticatedAsGuide(User user) = _AuthenticatedAsGuide;
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> signInAsGuide(String email, String password) async {
    state = const AuthState.loading();
    final result = await _repository.signIn(email: email, password: password);
    state = result.fold(
      (failure) => AuthState.error(failure.toString()),
      (user) => AuthState.authenticatedAsGuide(user),
    );
  }

  Future<void> signUp(String email, String password, String name) async {
    state = const AuthState.loading();
    final result = await _repository.signUp(email: email, password: password, name: name);
    state = result.fold(
      (failure) => AuthState.error(failure.toString()),
